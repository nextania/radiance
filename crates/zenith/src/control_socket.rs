...
impl ControlSocket {
    ...
    pub async fn set_http_challenge(
        ...
        let command = radiance_types::ControlCommand::SetHttpChallenge {
        ...
        let response: radiance_types::ControlResponse = serde_json::from_str(&line)?;
        match response {
            radiance_types::ControlResponse::Success { .. } => Ok(()),
            radiance_types::ControlResponse::Error { error } => Err(anyhow::anyhow!(error)),
        }
    }

    pub async fn clear_http_challenge(&self, domain: &str, token: &str) -> anyhow::Result<()> {
        let mut stream = UnixStream::connect(&self.socket_path).await?;
        let command = radiance_types::ControlCommand::ClearHttpChallenge {
            domain: domain.to_string(),
            token: token.to_string(),
        };
        let command_json = serde_json::to_string(&command)? + "\n";
    ...
    pub async fn send_reload_command(&self) -> anyhow::Result<()> {
        let mut stream = UnixStream::connect(&self.socket_path).await?;
        let command = radiance_types::ControlCommand::Reload;
        let command_json = serde_json::to_string(&command)? + "\n";
        stream.write_all(command_json.as_bytes()).await?;
